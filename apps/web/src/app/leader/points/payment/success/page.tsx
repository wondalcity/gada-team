...
import { workerPointsApi } from "@/lib/chat-api";
import { useT } from "@/lib/i18n";

function LeaderPaymentSuccessContent() {
  ...
  const mutation = useMutation({
    mutationFn: () => workerPointsApi.confirmCardPayment(paymentKey, orderId, amount),
  });
      ...
      <div className="w-full max-w-sm bg-white rounded-2xl border border-neutral-200 shadow-sm p-8 text-center">
        ...
        {mutation.isSuccess && (
          <>
            ...
            <button
              onClick={() => router.replace("/leader/points")}
...
export default function LeaderPaymentSuccessPage() {
  return (
    <Suspense>
      <LeaderPaymentSuccessContent />
    </Suspense>
  );
}
