...
export async function POST(
  ...
  try {
    ...
    if (invitation.user_id !== principal.userId) return forbidden("본인의 초대만 거절할 수 있습니다");
    if (invitation.invitation_status !== "PENDING") {
      return badRequest("대기 중인 초대만 거절할 수 있습니다");
    }
    ...
    if (updateError) {
      console.error("[invitations/[id]/reject POST] error:", updateError);
      return serverError();
    }
    ...
    console.error("[invitations/[id]/reject POST] error:", err);
    return serverError();
  }
}
