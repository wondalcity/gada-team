...
// POST /api/v1/jobs/[jobPublicId]/bids/[bidPublicId]/select
// Employer selects a winning bid — notifies all bidders
export async function POST(
  req: NextRequest,
  { params }: { params: Promise<{ jobPublicId: string; bidPublicId: string }> }
) {
  try {
    ...
    const { jobPublicId, bidPublicId } = await params;
    ...
    // Resolve user_id for selected bidder
    const selectedUserId = await resolveBidderUserId(db, selectedBid.bidder_type, selectedBid.bidder_user_id, selectedBid.bidder_team_id);
    if (selectedUserId) {
    ...
    if (rejectedBids && rejectedBids.length > 0) {
      const notifInserts = await Promise.all(
        rejectedBids.map(async (b) => {
          const uid = await resolveBidderUserId(db, b.bidder_type, b.bidder_user_id, b.bidder_team_id);
          return uid ? {
            user_id: uid,
            type: "BID" as const,
            title: "입찰 미선정 알림",
