...
export async function GET(req: NextRequest) {
  try {
    ...
    const content = (data ?? []).map((job) => {
      const site = job.sites as unknown as { public_id: string; name: string; address: string; sido: string | null; sigungu: string | null; companies: { public_id: string; name: string; logo_url: string | null; is_verified: boolean } };
