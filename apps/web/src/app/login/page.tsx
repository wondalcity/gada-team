            ...
            <li key={c.code}>
              <button
                ...
                className={`flex w-full items-center gap-3 px-3 py-2.5 text-left text-sm transition-colors ${
                  isSel ? "bg-primary-50" : "hover:bg-neutral-50"
                }`}
              >
                <span className="text-lg leading-none">{c.flag}</span>
                <span className="flex-1 font-medium text-neutral-900">{c.name}</span>
                <span className="tabular-nums text-neutral-400">{c.dial}</span>
                {isSel && (
                  <svg className="w-4 h-4 shrink-0 text-primary-500" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
